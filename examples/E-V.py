# -*- coding: utf-8 -*-
"""
Created on Tue Dec 31 07:08:59 2024
...
if __name__ == "__main__":    
    ...
    #================================== Plot ======================================
    fig_index = 1
    for j in range(len(lam_array)*len(V_0_array)):
        fig, ax = plt.subplots(figsize=(fig_width,fig_height))
        ax.tick_params(labelsize=32)
        plt.figure(fig_index)
        fig_index += 1
        sc = plt.scatter(E_array_real[j,:], E_array_imag[j,:], c=-np.log(IPR_array[j,:])/np.log(H_size))
        plt.title(f'V={V_0_array[j]:.2f},$t_x$={tx}, L={L},ChainNumber={S},BC={BC},$V(i)=V_0*e^{{1.0j*(2*\\pi*\\alpha*(i-(i//L)*L+1)+\\phi_3)}}$', fontsize=20)
        cbar = plt.colorbar()
        cbar.set_label('Tau',fontsize=36)    #Tau=-np.log(IPR)/np.log(H_size)    #log₁₀(IPR)
        cbar.ax.tick_params(labelsize=30)
        plt.xlabel('Re(E)',fontsize=42)
        plt.ylabel('Im(E)',fontsize=42)
        plt.ylim(-1.5,1.5)
        plt.savefig(fr'{current_dir}\E-V_save\V={V_0_array[j]:.2f}.png')
