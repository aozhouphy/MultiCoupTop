# -*- coding: utf-8 -*-
"""
Created on Tue Dec 31 07:45:11 2024
...
tx = 0.2                                          # 交叉跃迁
t2 = 0                                            # 垂直跃迁
V_0_array = np.arange(1.4, 1.6*t1+0.0001, 0.02)  # 无序势强度
...
if __name__ == "__main__":    
    ...
    # 如果索引index是sL-2, sL-1, sL 或 sL+1中的任意一个，且0≤S≤L，且0≤index≤SL-1，则该本征矢为局域边缘态。
    ...
    for j in range(len(V_0_array)):                # 索引为j的参数V_0
        ...
        for q in range(0,H_size,1):                # 取出其中第q个本征矢
            ...
            index_array = np.argsort(P_q)          # 对概率从小到大排序
            max_0 = P_q[index_array[-1]]           # 取出最大的三个概率
            ...
            if max_0 > LS*(max_1+max_2):           # 满足这个条件的认为是局域态（Localized State）
               if index_array[-1] in edge_site_list:
                   # Plot
                    ...
                    fig_index += 1
                    sc = plt.plot(range(0,H_size,1),np.log(P_q))
                    ...
                    plt.xlabel('Site',fontsize=42)
                    plt.ylabel('$ln(P)$',fontsize=42)
